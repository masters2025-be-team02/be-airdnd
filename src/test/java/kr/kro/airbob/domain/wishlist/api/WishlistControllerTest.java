...
class WishlistControllerTest extends BaseControllerDocumentationTest {
	...
	void 사용자가_새로운_위시리스트를_생성한다() throws Exception {
		...
		WishlistDto.createRequest request = new WishlistDto.createRequest("서울 여행 계획");
		WishlistDto.createResponse expectedResponse = new WishlistDto.createResponse(1L);

		when(wishlistService.createWishlist(any(WishlistDto.createRequest.class), eq(1L)))
		...
		verify(wishlistService).createWishlist(any(WishlistDto.createRequest.class), eq(1L));
	}
	...
	void 사용자가_빈_이름으로_위시리스트_생성을_시도한다() throws Exception {
		...
		WishlistDto.createRequest invalidRequest = new WishlistDto.createRequest("");
	...
	void 사용자가_이름을_입력하지_않고_위시리스트_생성을_시도한다() throws Exception {
		...
		WishlistDto.createRequest invalidRequest = new WishlistDto.createRequest(null);
	...
	void 사용자가_255자_초과_이름으로_위시리스트_생성을_시도한다() throws Exception {
		...
		WishlistDto.createRequest invalidRequest = new WishlistDto.createRequest(longName);
	...
	void 여러_개의_유효한_위시리스트를_연속으로_생성한다() throws Exception {
		...
		for (int i = 0; i < wishlistNames.length; i++) {
			WishlistDto.createRequest request = new WishlistDto.createRequest(wishlistNames[i]);
			WishlistDto.createResponse expectedResponse = new WishlistDto.createResponse((long)(i + 1));
	...
	void 동일한_이름으로_여러_위시리스트를_생성한다() throws Exception{
		...
		WishlistDto.createRequest request = new WishlistDto.createRequest("초복중복말복");
		WishlistDto.createResponse firstResponse = new WishlistDto.createResponse(1L);
		WishlistDto.createResponse secondResponse = new WishlistDto.createResponse(2L);
